...
3. **Vendor.** The skill is copied into this repository at a pinned version, with a
   `PROVENANCE.md` recording where it came from and who approved it.
...
above, except that it is authored rather than imported: `PROVENANCE.md` names you and the date
...
| `plugins/<name>/` | One vendored plugin: its skills, its `PROVENANCE.md`, its scan baseline |
